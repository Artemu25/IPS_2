...
void ParallelSortWithTime(int *begin, int *end, long count) {
...
void CompareForAndCilk_For(size_t sz) {
	...
	high_resolution_clock::time_point t2 = high_resolution_clock::now();
	duration<double> duration = (t2 - t1);
	std::cout << "Duration for usual vector is: " << duration.count() << " seconds" << std::endl;
	...
	duration = (t2 - t1);
	std::cout << "Duration for reduce_vector is: " << duration.count() << " seconds" << std::endl << std::endl;
}
...
int main()
{
	...
	//ParallelSortWithTime(mass_begin, mass_end, mass_size);

	long sizes[8] = { 1000000, 100000, 10000, 1000, 500, 100, 50, 10};
	
	for (auto element: sizes) {
		CompareForAndCilk_For(element);
	}
